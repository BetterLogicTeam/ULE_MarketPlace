...
import Loading_Spinner from '../Loading_Spinner/Loading_Spinner';
...
export default function AuctionsTwo() {
  ...
  const auction = async () => {
    ...
    console.log("Api Call Auction", e);
  }
    ...
    <div class="tf-section sc-explore-1">
      <div class="container">
        <div class="row">
          ...
          {
            ApiData == "" ? <Loading_Spinner/>:ApiData?.map((items, index) => {
              return (
                <>
                  <div class="fl-item col-xl-4 col-lg-4 col-md-6 col-sm-6" style={{ display: "block", cursor: "pointer" }}>
                    <div class="sc-card-product" onClick={() => myHistory.push(`/AuctionModal/${"All"}/${index}`)}>
